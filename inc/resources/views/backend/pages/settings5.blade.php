    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open pt-2">
            <a href="{{route('settings5',[$phone])}}" class="nav-link active">
              <i class="nav-icon fas fa-solid fa-briefcase"></i>
              <p>
                References
                <i style="padding-top: 15px;" class="right fas fa-angle-down"></i>
...
<div id="workExperience">
  <fieldset>
    <div class="box-border ">
      @foreach ($company as $key => $value)
      <br>
      <h3 class="text-center">References {{$key+1}}</h3>
        <form action="{{route('referenceUpdate')}}" method="POST">
          @csrf
          <div class="row">       
            <input type="hidden" name="id" value="{{$usersDetails[$key]['id']}}">
            <input type="hidden" name="number" value="{{$usersDetails[$key]['number']}}">         
            <div class="col-md-6 col-sm-12">
                <label for="name"><b> Name : </b></label>
                <input type="text" placeholder="Enter Name" name="name" id="name" value="{{$usersDetails[$key]['name']}}">
                @error('name')
            ...
            <div class="col-md-6 col-sm-12">  
                ...
                <input type="text" placeholder="Designation" name="designation" id="designation" value="{{$usersDetails[$key]['designation']}}">
                @error('designation')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6 col-sm-12">
                ...
                <input type="text" placeholder="Enter Company Name" name="company_name" id="company_name" value="{{$usersDetails[$key]['company_name']}}">
                @error('company_name')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6 col-sm-12">
                ...
                <input type="tel" placeholder="01*********" name="mobile" id="mobile" pattern="[0-1]{2}[0-9]{9}" value="{{$usersDetails[$key]['mobile']}}">
                @error('mobile')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6 col-sm-12">  
                ...
                <input type="email" placeholder="Email" name="email" id="email" value="{{$usersDetails[$key]['email']}}">
                @error('email')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6 col-sm-12">  
                ...
                <input type="text" placeholder="Address" name="address" id="address" value="{{$usersDetails[$key]['address']}}">
                @error('address')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
